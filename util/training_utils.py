...
from util.helper import load_mongo_docs, filter_rejected_docs
...
def get_corpus_data(db_host: str, db_port: int, input_collection: str, random_state: int, train_perc: float, corpus_type: str, min_len: int, unique_by_metadata=True, include_reject=False):
    data = load_mongo_docs(min_len, unique_by_metadata, input_collection, db_host, db_port)
    if not include_reject:
        data = filter_rejected_docs(data)
